...
type RequestRow = { id: string; status: string; requested_at: string; assigned_at: string | null; cancelled_at: string | null; cancellation_reason: string | null; patient: EmbeddedPatient | EmbeddedPatient[] | null; appointment: EmbeddedAppointment | EmbeddedAppointment[] | null };
...
export async function POST(request: Request) {
  ...
  const { data, error } = authorization.clinicalRole === "ADMINISTRATIVE"
    ? await supabase.rpc("rpc_create_initial_appointment_request_for_patient", { p_patient_id: patientId })
