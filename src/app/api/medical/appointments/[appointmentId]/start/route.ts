...
export async function POST(_request: Request, context: { params: Promise<{ appointmentId: string }> }) {
  const authorization = await requireApiRole("REVIEW_DOCTOR");
  ...
  const { data: appointment, error: appointmentError } = await supabase.from("appointment").select("id,patient_id,status,appointment_type").eq("id", appointmentId).eq("assigned_staff_id", staff.id).maybeSingle();
  if (appointmentError) return supabaseError(appointmentError);
  if (!appointment || appointment.appointment_type !== "INITIAL") return NextResponse.json({ ok: false, error: "La cita inicial no pertenece a este médico." }, { status: 404 });
  ...
  const { data: encounterId, error } = await supabase.rpc("rpc_open_encounter", { p_patient_id: appointment.patient_id, p_appointment_id: appointmentId, p_referral_id: null, p_chief_complaint: "Pendiente de entrevista clínica." });
